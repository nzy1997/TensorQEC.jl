...
function compile(decoder::SimulatedAnnealing, problem::IndependentDepolarizingDecodingProblem)
...
function sa_energy_hard(ops_config::Vector{Bool}, sap::SpinGlassSA, bit_config::Vector{Mod2})
    @assert length(ops_config) == length(sap.ops)
