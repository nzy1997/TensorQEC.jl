...
# We can generate the truth table for the error correction by [`make_table`](@ref). The truth table is a dictionary, where the key is the syndrome, and the value is the error Pauli string. We can print the truth table by [`show_table`](@ref).
...
# The measrue outcome shows that the stabilizer 4 and 6 are violated. According to the truth table, the error is $X_5$, which is exactly the error we applied.
