...
@testset "compile and decode CUDA" begin   
    d = 3
    # tanner = CSSTannerGraph(ToricCode(d,d))
    tanner = CSSTannerGraph(SurfaceCode(d,d))
    n = tanner.stgx.nq
...
@testset "sa_energy" begin
    ...
    energyold =  TensorQEC.sa_energy(config, prob)
    energydiff =  TensorQEC.read_tensor_deltaE(TensorQEC.getview(prob.logp,1),1,config[[1,d*d+1]])
    config[1] += Mod2(1)
